...
public class SongH2Service implements SongRepository {
  ...
  public ArrayList<Song> getSongs() {
    ...
    List<Song> songData = db.query("select * from playlist", new SongRowMapper());
    ArrayList<Song> songs = new ArrayList<>(songData);
    return songs;
  ...
  public Song getSongById(int songId) {
    try {
      Song song = db.queryForObject("select * from playlist where songId = ?", new SongRowMapper(),
          songId);
      return song;
    } catch (Exception e) {
